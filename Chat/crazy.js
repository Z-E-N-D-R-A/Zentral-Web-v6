...
firebase.auth().onAuthStateChanged(async u => {
  ...
  if (saved) {
    ...
    document.getElementById("crazy-root").classList.add("hidden");
    return;
  }
  ...
  document.getElementById("crazy-root").classList.remove("hidden");
  document.getElementById("blocked").style.display = "none";
  document.getElementById("crazy-status").textContent = accountId;
...
async function cleanupOldMessages(days = 30) {
  ...
  if (!confirm(`Delete ${ids.length} messages older than ${days} days?`)) {
    cleanupResult.textContent = "Cleanup cancelled.";
    return;
  }
...
async function cleanupInactiveAccounts(days = 90) {
  ...
  if (!confirm(`Delete ${ids.length} inactive accounts?`)) {
    cleanupResult.textContent = "Cleanup cancelled.";
    return;
  }
